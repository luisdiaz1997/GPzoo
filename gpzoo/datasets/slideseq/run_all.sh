...
# GPU 1: LCGP models (adding to GPU 0 list)
echo "Starting LCGP (GPU 0)..."
CUDA_VISIBLE_DEVICES=0 $PYTHON -m gpzoo.datasets.slideseq.lcgp_nsf &
PID5=$!

# GPU 1: MGGP-LCGP models (adding to GPU 1 list)
echo "Starting MGGP-LCGP (GPU 1)..."
CUDA_VISIBLE_DEVICES=0 $PYTHON -m gpzoo.datasets.slideseq.lcgp_mggp_nsf &
PID6=$!

echo ""
echo "All jobs started in background!"
echo "Process IDs: $PID1 $PID2 $PID3 $PID4 $PID5 $PID6"
echo ""
...
echo "  kill $PID1 $PID2 $PID3 $PID4 $PID5 $PID6"
echo ""
