...
def _train_main() -> None:
    ...
    for h5_path in h5_files:
        X, Y, _, _, V_init = load_visium_with_regions(h5ad_path=h5_path)

        writer = SummaryWriter(log_dir=OUTPUT_DIR / "tb" / f"{h5_path.stem}_vnngp_k={VNNGP_K}")

        checkpoint_path = OUTPUT_DIR / f"{h5_path.stem}_vnngp_k={VNNGP_K}.pth"
        checkpoint_arg = str(checkpoint_path) if checkpoint_path.exists() else None
        start_step = 0

        model, _ = build_model(
            "tenxvisium/vnngp_nsf",
        ...
        result = run_training(
            label=f"{h5_path.stem}_vnngp_k={VNNGP_K}",
            model=model,
            optimizer=optimizer,
            ...
            writer=writer,
            progress_desc=f"VNNGP ({h5_path.stem})",
