...
def run_experiment(X, Y, save_path, NMF=True, batched=True):
    ...
    for params in ParameterGrid(param_grid):
        ...
        if kwargs['L'] == 5:
            fig2, ax = putil.plot_factors_five(mean, X.cpu().detach().numpy(), moran_idx=moran_idx, size=1.5, s=0.005, alpha=0.9, ax=None)
        else:
            fig2, ax = putil.plot_factors(mean, X.cpu().detach().numpy(), moran_idx=moran_idx, size=1.5, s=0.005, alpha=0.9, ax=None)
        #fig2.suptitle("Slideseq NNNSF", size=15)
