...
def run_experiment(X, Y, save_path, NMF=True, batched=True):
    ...
    csv_path = path.join(nsf_path, 'nsfh_csv_results.csv')

    file_exists = path.isfile(csv_path)
    if not file_exists:
        headers = ['learning_rate', 'sigma', 'lengthscale', 'ips', 'mean_cv_loss']
    ...
    with open(csv_path, mode='a', newline='') as file:
        ...
        for params in ParameterGrid(param_grid):
            ...
            writer.writerow([params['learning_rate'], params['sigma'], params['lengthscale'], kwargs['M'], mean_cv_loss])
...
def main():
    ...
    save_path = path.join(SPATH, 'nsfh/validation_experiments')
